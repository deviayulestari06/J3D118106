    ...
    if ($_GET['aksi']=="Edit"){
        ...
        if (isset($_GET['sub'])){
            if ($_GET['sub']=="Kembali ke Tampil Data"){
                ...
                if (strlen($_GET['nm'])){
                    include "koneksi.php";
                    mysqli_query($kon,"UPDATE `mahasiwa` SET `nama` = '".$_GET['nm']."' WHERE `mahasiwa`.`id` = ".$_GET['idupdate']);
                    //echo "UPDATE `mahasiwa` SET `nama` = '".$_GET['nm']."' WHERE `mahasiwa`.`id` = ".$_GET['idupdate'];
        ...
        if (isset($_GET['sub'])){
          if($_GET['sub']=="tidak"){
            ...
            include "koneksi.php";
            mysqli_query($kon,"DELETE FROM `mahasiwa` WHERE `id` = ".$_GET['id']);
            header("location:01_tampildata.php");
          }
        }
    }
